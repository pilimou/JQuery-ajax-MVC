...
<title>ajax+bootstrap+page</title>
</head>
<body>
	...
	<div id="showPage" style="width:80%;display:flex;justify-content:center"></div>
	<!-- 當前頁數 -->
	<input id="nowPage" type="text" style="display:none">
...
// 		$('input[name=sexGroup]:checked').val();
		...
		//ajax
		var processMethod = "queryEmpPage";
		var page = "1"; 												//重新查詢預設第一頁
		$("#nowPage").val(page);										//紀錄當前頁數
		var processURL = "/mvcExercise/" + processMethod + "/" + page;
		initSubmitForm(processURL, processMethod, Employee);
	...
	$('#dialog-insertbtn').click(function(){
		...
		//ajax
		var processMethod = "insertEmp";
		var processURL = "/mvcExercise/" + processMethod + "/";
		initSubmitForm(processURL, processMethod, Employee);
	...
	$("#dialog-deletebtn").click(function(){
		...
		//ajax
		var processMethod = "deleteEmp";
		var processURL = "/mvcExercise/" + processMethod + "/";
		initSubmitForm(processURL, processMethod, Employee, Employee.id);
	});
...
//=========================== ajax交換資料 ===================================
	...
	//ajax傳送接收json
	function initSubmitForm(processURL, processMethod, processData, pkId) {
          $.ajax({
              url: processURL,
              type: "POST",
              datatype: "json",
              ...
              success: function(data) {
                  ...
                  //成功後判斷要給哪隻方法處理
              	if("deleteEmp" === processMethod){
                  	
                	deleteDataMapping(pkId);
                	
                } else if("insertEmp" === processMethod){
                    
                	insertDataMapping(data);	
                	
                } else if("queryEmpPage" === processMethod){

                	queryDataMapping(data);
                	
                } else if("updateEmp" === processMethod){
...
//===========================解包json寫入標籤資料=============================	
	...
	function queryDataMapping(data){
		...
		var empCount = dataList.queryCount;			//該查詢條件下總資料筆數
		var CountPerPage = dataList.countPerPage;	//一頁顯示幾筆
		var empList = dataList.employee;			//該查詢條件下的資料
		...
		var totalPages = Math.ceil(empCount/CountPerPage); //總頁數 = (取最小整數) 總資料筆數/每頁幾筆

		var pageSegment = empPage(totalPages);  //組合頁數標籤

		$("#showPage").html(pageSegment);
	...
	//組合頁數標籤(傳入參數:總頁數; 回傳:頁數標籤組合)
	function empPage(totalPages){
		var pageSegment = "";
		var nextPage = "";
		var prePage = "";
		var nowPage = $("#nowPage").val();		//當前頁數
		...
		pageSegment += "<button class=\"pageBtn\" value=\"1\">第一頁</button>&nbsp";

		pageSegment += "<button class=\"pageBtn\" value=\"" + prePage + "\">上一頁</button>&nbsp";
		
		for(var p = 1; p <= totalPages; p++){
			pageSegment += "<button class=\"pageBtn pageFocus\" value=\"" + p + "\">" + p + "</button>&nbsp";
		}

		pageSegment += "<button class=\"pageBtn\" value=\"" + nextPage + "\">下一頁</button>&nbsp";

		pageSegment += "<button class=\"pageBtn\" value=\"" + totalPages + "\">最末頁</button>&nbsp";
		
		return pageSegment;
	}
	
	//按下頁數
	$("#showPage").on('click', '.pageBtn', function(){
